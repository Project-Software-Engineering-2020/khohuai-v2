...
const addMyCart = async (req, res) => {
    const uid = "kg7nruHErBXDT03u5Bdfb3bfSzb2";
...
const adjustMyCart = async (req, res) => {
  ...
  try {
    ...
    if (data.qty + qty <= inStock.stock) {
      ...
      res.status(200).send("ไม่มีสลากใบนี้อยู่");
...
const removeMyCart = async (req, res) => {
  ...
  try {
    ...
    await firestore
      ...
      .catch((err) =>
        console.log("ลบไม่ได้ ไม่รู้เป็นไร ดู error เอาเอง", err)
      );
